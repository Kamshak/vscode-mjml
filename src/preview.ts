...
export default class PreviewManager {
  ...
  private previewCommand(document?: vscode.TextDocument): void {
    ...
    if (!this.IDMap.hasUri(documentURI)) {
      mjmlPreview = new MJMLView(((document) ? document : vscode.window.activeTextEditor.document), this);
  ...
  removePreview(document: vscode.TextDocument): void {
...
class MJMLView {
  ...
  constructor(document: vscode.TextDocument, private previewManager: PreviewManager) {
  ...
  public execute(): void {
    try {
      if (!this.panel) {
        this.panel = vscode.window.createWebviewPanel('mjml.showPreview', 'Preview ' + this.document.uri.path.split('/').slice(-1)[0], vscode.ViewColumn.Two);
        this.subscriptions.push(
          this.provider.onDidChange((e: vscode.Uri) => {
            try {
              this.panel.webview.html = this.provider.provideTextDocumentContent(this.previewUri);
            } catch (e) {
              vscode.window.showErrorMessage(e.message);
            }
          }),
          this.panel.onDidDispose(() => {
            this.previewManager.removePreview(this.document);
          })
        );
      }
      this.panel.webview.html = this.provider.provideTextDocumentContent(this.previewUri);
  ...
  private renderMJML(): string {
    ...
    try {
      return renderMjml(
        ...
        {}, false, false, this.document.uri.fsPath
      );
    } catch (e) {
      ...
      if (error.name) {
        return `
                    <body style="background:#AD2222; padding: 20px; color: white">
                        ...
                        <p style="margin-bottom: 1rem">${error.name} - ${error.message}</p>
